...
@property (nonatomic ,strong) UILabel  * pickerTitle;/**< 标题*/
...
-(UILabel *)pickerTitle {
    if (!_pickerTitle) {
        _pickerTitle = [[UILabel alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-60)/2, 0, 60, TITLEHEIGHT)];
        _pickerTitle.font = [UIFont systemFontOfSize:14.f];
        _pickerTitle.text = @"地区选择";
    }
    return _pickerTitle;
}
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor:COLOR_WITH_HEX(0xdd5652)
                         forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        _sureBtn.titleLabel.font = [UIFont systemFontOfSize:14.f];
        [_sureBtn setTitleColor:COLOR_WITH_HEX(0x417293)
                       forState:UIControlStateNormal];
...
- (void)loadTitle{
    ...
    [self.titleBackgroundView addSubview:self.pickerTitle];
